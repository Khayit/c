...
int racket(char field[STR][ROW], int *left_racket,
           int *right_racket); // Rackets physics
void ball(char field[STR][ROW], int left_racket, int right_racket, int *ball_i,
          int *ball_j, int *dir_x,
          int *dir_y); // Ball physics
void goal(int *score, int *left_racket, int *right_racket, int *ball_i,
          int *ball_j, int *dir_x,
...
int main() {
  char field[STR][ROW];
  int left_racket = STR / 2,
      right_racket = STR / 2;             // Left and right rackets coordinates
  ...
  do {
    ...
    if (ball_j == 1)
      goal(&right_score, &left_racket, &right_racket, &ball_i, &ball_j, &dir_x,
           &dir_y);
    else if (ball_j == ROW - 2)
      goal(&left_score, &left_racket, &right_racket, &ball_i, &ball_j, &dir_x,
           &dir_y);
...
void ball(char field[STR][ROW], int left_racket, int right_racket, int *ball_i,
          int *ball_j, int *dir_x, int *dir_y) {
  if ((*ball_i) == 1 && (*dir_x) == 1 && (*dir_y) == -1)
    ...
    (*dir_y) *= -1;
           ...
           ((*ball_i) == left_racket || (*ball_i) == left_racket + 1 ||
            (*ball_i) == left_racket - 1 || (*ball_i) == left_racket + 2 ||
            (*ball_i) == left_racket - 2))
    (*dir_x) *= -1;
  else if ((*ball_j) == 2 && (*dir_x) == -1 && (*dir_y) == -1 &&
           ((*ball_i) == left_racket || (*ball_i) == left_racket + 1 ||
            (*ball_i) == left_racket - 1 || (*ball_i) == left_racket + 2 ||
    ...
    (*dir_x) *= -1;
           ...
           ((*ball_i) == right_racket || (*ball_i) == right_racket + 1 ||
            (*ball_i) == right_racket - 1 || (*ball_i) == right_racket + 2 ||
            (*ball_i) == right_racket - 2))
    (*dir_x) *= -1;
  else if ((*ball_j) == ROW - 3 && (*dir_x) == 1 && (*dir_y) == -1 &&
           ((*ball_i) == right_racket || (*ball_i) == right_racket + 1 ||
            (*ball_i) == right_racket - 1 || (*ball_i) == right_racket + 2 ||
